          ...
          <div className="my-1 w-[70%]">
            <section
              className={`flex w-[100%] ${
                menu == 0 ? 'justify-evenly' : null
              } mb-4 bg-[#BBB] p-3 rounded-xl`}
            >
              {!auth.currentUser ? (
                <h1 className="text-white font-eudoxus">
                  Login / Sign Up in order to create any posts.
                </h1>
              ) : menu == 0 ? (
                <>
                  <Link
                    to="/social/create"
                    className=" bg-gradient-to-r from-amber-600 to-amber-500 hover:scale-105 duration-300  w-[45%] rounded-lg shadow-xl py-2"
                  >
                    <h1 className="text-white font-eudoxus text-center">Create Post</h1>
                  </Link>
                  <button
                    className="text-white font-eudoxus bg-gradient-to-r from-blue-500 to-blue-400 hover:scale-105 duration-300  w-[45%] rounded-lg shadow-xl py-2"
                    onClick={() => {
                      setMenu(1)
                    }}
                  >
          ...
          <section className="w-3/12 bg-slate-500 p-1 rounded-xl shadow-lg">
            <div className="gap-3 flex flex-col p-3 h-[100%] items-center overflow-y-scroll no-scrollbar">
              ...
              {forums.map((c) => (
                <div
                  key={c}
                  className="w-[100%] py-3 cursor-pointer hover:scale-105 justify-center flex bg-slate-500 rounded-full hover:bg-slate-700 duration-200"
                >
